...
for (const ext of ["kicad_pro", "kicad_pcb", "kicad_dru"])
  cpSync(`${qa}/pcbnew/api_kitchen_sink.${ext}`, `${proj}/api_kitchen_sink.${ext}`);
...
page.on("framenavigated", (f) => {
  if (f === page.mainFrame()) console.log("!! main frame navigated to", f.url());
});
page.on("console", (m) => {
  const t = m.text();
  if (/[vite]|reload|WebGL|CONTEXT_LOST|context lost/i.test(t)) console.log("console[" + m.type() + "]:", t.slice(0, 200));
});
...
const count = (key, type) =>
  kw(
    ({ key, type }) => {
      const d = window.__fpPcb.services.documents;
      const st = key === "board" ? d.board() : d.sheet(key);
      return [...st.byType(type)].length;
    },
    { key, type },
  );
...
try {
  ...
  if (want("board")) {
    ...
    log(
      "selected:",
      (await page.locator(".statusbar .msg").innerText()).trim(),
      "| props title:",
      (await page.locator(".props-title").innerText()).replace(/\s+/g, " "),
    );
  ...
  if (want("edit")) {
    ...
    const padsAt = (txt) => {
      const i = txt.indexOf('(footprint "Resistor_SMD:R_0603_1608Metric"');
      const seg = txt.slice(i, i + 4000);
      return [...seg.matchAll(/\(pad "(\d)"[\s\S]*?\((?:at|translate) (-?[\d.]+) (-?[\d.]+)/g)]
        .map((m) => `${m[1]}@${m[2]},${m[3]}`)
        .join(" ");
    };
    const fpAt = (txt) => {
      const i = txt.indexOf('(footprint "Resistor_SMD:R_0603_1608Metric"');
      return /\((?:at|translate) (-?[\d.]+) (-?[\d.]+)/
        .exec(txt.slice(i, i + 600))
        ?.slice(1, 3)
        .join(",");
    };
  ...
  if (want("route")) {
    ...
    log(
      `route: tracks ${tracks0} -> ${tracks1}, vias ${vias0} -> ${vias1}; file has ${segs} segments, via at (110,100) F.Cu-B.Cu: ${viaLine}; toast: ${await notify()}`,
    );
    await page.screenshot({ path: `${shots}/board-route.png` });
    record(
      "route",
      tracks1 === tracks0 + 3 && vias1 === vias0 + 1 && viaLine,
      `3 segments + via written by KiCad (revision ${r0} -> ${await rev()})`,
    );
  ...
  if (want("draw")) {
    ...
    const zone = await kw(() => {
      const z = [...window.__fpPcb.services.documents.board().byType("KOT_PCB_ZONE")].find((x) => x.proto.name === "proof-zone");
      return z
        ? { filled: z.proto.filled, fills: z.proto.filledPolygons.length, net: z.proto.settings?.value?.net?.name, layers: z.proto.layers }
        : null;
    });
    ...
    record(
      "draw",
      shapes1 === shapes0 + 5 && texts1 === texts0 + 1 && zones1 === zones0 + 1 && zone?.filled === true,
      `line/rect/circle/arc/polygon + text + zone (filled after RefillZones: ${zone?.fills} polygon sets on ${zone?.net})`,
    );
  }
  ...
  if (want("footprint")) {
    ...
    await page.waitForFunction(
      () => /Footprint: click/.test(document.querySelector('[data-testid="tool-hint"]')?.textContent ?? ""),
      null,
      { timeout: 30000 },
    );
    ...
    const placed = await kw((ref) => {
      const f = [...window.__fpPcb.services.documents.board().byType("KOT_PCB_FOOTPRINT")].find(
        (x) => x.proto.referenceField?.text?.text?.text === ref,
      );
      return f
        ? {
            at: [Number(f.proto.position.xNm) / 1e6, Number(f.proto.position.yNm) / 1e6],
            items: f.proto.definition?.items?.length,
    ...
    const pads =
      i > 0
        ? [...s.slice(i, i + 6000).matchAll(/\(pad "(\d)"[\s\S]*?\((?:at|translate) (-?[\d.]+) (-?[\d.]+)/g)]
            .map((m) => `${m[1]}@${m[2]},${m[3]}`)
            .join(" ")
        : "not in file";
    log(
      `footprint: ${fps0} -> ${fps1}; ${ref} = ${JSON.stringify(placed)}; file pads ${pads}; sessions on bridge: ${(await (await fetch(`${bridge}/sessions`)).json()).sessions.length} (library session spawned)`,
    );
    await page.screenshot({ path: `${shots}/board-footprint.png` });
    record(
      "footprint",
      fps1 === fps0 + 1 && placed?.items >= 2 && pads.includes("1@-0.825,0"),
      `${ref} ${placed?.lib} at ${placed?.at} with ${placed?.items} definition items; pads in file: ${pads}`,
    );
    record(
      "library-place",
      previewCanvas === 1,
      `library browser: entry picked from the fp-lib-table listing, ${previewCanvas} preview canvas, placed as ${ref}`,
    );
  }
  ...
  if (want("align")) {
    ...
    const before = await kw(
      (ids) =>
        ids.map((id) => {
          const f = window.__fpPcb.services.documents.board().get(id);
          return [Number(f.proto.position.xNm) / 1e6, Number(f.proto.position.yNm) / 1e6];
        }),
      ids,
    );
    const r0 = await rev();
    ...
    await waitRev(r1);
    const after2 = await kw(
      (ids) =>
        ids.map((id) => {
          const f = window.__fpPcb.services.documents.board().get(id);
          return Number(f.proto.position.xNm) / 1e6;
        }),
      ids,
    );
    ...
    const track = await kw(() => {
      const t = [...window.__fpPcb.services.documents.board().byType("KOT_PCB_TRACE")][0];
      return { id: t.id, layer: t.layer, net: t.net };
    });
    ...
    await waitRev(r4);
    const trackAfter = await kw((id) => {
      const t = window.__fpPcb.services.documents.board().get(id);
      return { layer: t.layer, net: t.net };
    }, track.id);
    ...
    record(
      "align",
      sameY && orient === 45 && trackAfter.layer !== track.layer && trackAfter.net === "PROOF_NET",
      `align top → y=${after[0][1]}, distribute, rotate 45°, set layer ${trackAfter.layer}, set net ${trackAfter.net}`,
    );
    await focusCanvas();
    for (let i = 0; i < 5; i++) {
      const r = await rev();
  ...
  if (want("clipboard")) {
    ...
    log(
      `KiCad text paste: SaveItemsToString ${sexpr.length} chars (${sexpr.slice(0, 40).replace(/\s+/g, " ")}…) -> ParseAndCreateItemsFromString: tracks ${n1} -> ${n2}; undo -> ${n3}`,
    );
    record(
      "clipboard",
      n1 === n0 + 2 && n2 === n1 + 1 && n3 === n1,
      "duplicate (Mod+D), copy/paste (Mod+C / Mod+V at cursor) through CreateItems, and SaveItemsToString → ParseAndCreateItemsFromString paste + undo",
    );
  }
  ...
  if (want("markers")) {
    ...
    if (rows > 0) {
      ...
      const overlay = await kw(() => {
        const h = window.__fpPcb.host("board");
        return typeof h.setMarkers === "function" && typeof h.focusMarker === "function";
      });
      ...
      log(
        `DRC: ${rows} markers, overlay API ${overlay}, selection after focus ${sel}, excluded ${before} -> ${after.length} (${JSON.stringify(after[0])})`,
      );
      record(
        "markers",
        overlay && after.length === before + 1,
        `${rows} markers fed to host.setMarkers, focusMarker + selection on click, exclusion comment "${after[0]}" round-tripped through SetDrcMarkerExcluded`,
      );
    ...
    await page.waitForTimeout(1500);
    const now = await kw(
      async ({ rule }) => (await window.__fpPcb.services.board.severities("drc")).find((r) => r.rule === rule)?.severity,
      { rule },
    );
  ...
  if (want("nets")) {
    ...
    const statusText = await page
      .locator('[data-testid="unrouted-count"]')
      .innerText()
      .catch(() => "");
    ...
    await page
      .locator(".left-rail button, .panel-tabs button", { hasText: "Nets" })
      .first()
      .click()
      .catch(() => {});
    ...
    log(
      `nets: unrouted ${JSON.stringify(unrouted)}, status bar "${statusText.replace(/\s+/g, " ")}", ratsnest toggle ${on}/${off}, ${lengthRows} length rows (top by length ${top}, by pads ${byPads})`,
    );
    await kw(() => window.__fpPcb.host("board").zoomToFit());
    await page.waitForTimeout(400);
    record(
      "nets",
      lengthRows > 0 && off === false && on === true && /unrouted/.test(statusText),
      `GetNetLengths ${lengthRows} rows with sortable columns, GetUnroutedCount ${unrouted.unroutedCount} in the status bar, ratsnest toggle works`,
    );
  }
  ...
  if (want("serverundo")) {
    ...
    await focusCanvas();
    await run("board.placeVia");
    await page.waitForFunction(() => /Via/.test(document.querySelector('[data-testid="tool-hint"]')?.textContent ?? ""), null, {
      timeout: 20000,
    });
    ...
    log(
      `server undo: mode "${mode}", vias ${vias0} -> ${vias1} -> ${vias2}; KiCad stack ${JSON.stringify(stack.undo.map((e) => e.description))} -> ${JSON.stringify(after.undo.map((e) => e.description))}, revision ${r0} -> ${r1}`,
    );
    record(
      "serverundo",
      /server undo/.test(mode) && vias1 === vias0 + 1 && vias2 === vias0 && stack.undo.length > after.undo.length,
      `history panel in ${mode}; GetUndoStack showed ${stack.undo.length} entries, Undo removed the via and popped one`,
    );
  }
  ...
  if (want("settings")) {
    ...
    const defaults = (
      await page
        .locator('[data-testid="kicad-defaults"]')
        .innerText()
        .catch(() => "")
    ).replace(/\s+/g, " ");
    if (themes.length) {
    ...
    record(
      "settings",
      themes.length > 0 && applied.startsWith("server:") && /Units/.test(defaults),
      `${themes.length} KiCad themes offered, canvas pinned to ${applied}; GetAppSettings defaults shown (${defaults.slice(0, 90)})`,
    );
  }
  ...
  if (want("setup")) {
    ...
    log(
      `board setup: clearance ${before.rules.minClearanceNm} -> ${after.rules.minClearanceNm}; F.Cu ${cu?.material} ${cu?.thicknessNm}; file stackup F.Cu thickness ${stack?.[1]} material "${stack?.[2]}"; rule ${after.customRuleList[0]?.name}; grid origin ${JSON.stringify(after.origin.grid)}`,
    );
    record(
      "setup",
      after.rules.minClearanceNm === 123000 &&
        cu?.thicknessNm === 70000 &&
        after.origin.grid.x === 12_500_000 &&
        (!hadRule || after.customRuleList[0]?.name === "proof-rule"),
      `SetBoardDesignRules + UpdateBoardStackup + SetCustomDesignRules + SetBoardOrigin round-trip (${err || "no error"})`,
    );
  }
  ...
  if (want("page")) {
    ...
    await page.waitForSelector(".dialog", { timeout: 5000 });
    await page.waitForFunction(
      () => document.querySelector('[data-page="title"]') || document.querySelector(".dialog [role=alert]"),
      null,
      { timeout: 15000 },
    );
    const errB = await page
      .locator(".dialog [role=alert]")
      .innerText()
      .catch(() => "");
    let tb = null;
    if (!errB) {
      ...
      tb = await kw(async () => {
        const b = window.__fpPcb.services.documents.boardDoc;
        return { title: (await b.titleBlock()).title, page: (await b.pageSettings()).pageSize };
      });
      const s = await saved();
      log(
        `board page settings: title block now ${JSON.stringify(tb)}; file: ${/\(paper "([^"]+)"\)/.exec(s)?.[1]} title "${/\(title "([^"]*)"\)/.exec(s)?.[1]}"`,
      );
    } else {
      log("board page settings refused:", errB);
      await page.keyboard.press("Escape");
    }
  ...
  if (want("3d")) {
    ...
    await page.waitForFunction(
      () => /mesh|failed|unavailable/i.test(document.querySelector('[data-testid="three-status"]')?.textContent ?? ""),
      null,
      { timeout: 180000 },
    );
    await page.waitForTimeout(1500);
  ...
  if (want("jobs")) {
    ...
    for (const [id, opts] of [
      ...
      const r = await kw(
        async ({ id, opts }) => {
          ...
          const run = await j.run(id, { ...o, ...opts });
          return {
            state: run.state,
            error: run.error,
            outputs: run.outputs.map((f) => `${f.name} (${(f.bytes / 1024).toFixed(1)} KiB)`),
            log: run.log,
          };
        },
        { id, opts },
      ...
      if (async) asyncRuns++;
      outcomes.push(
        `${id}: ${r.state}${r.error ? ` (${r.error.slice(0, 120)})` : ""}${async ? ` [async, ${progress} progress lines]` : " [sync]"} ${r.outputs.join(", ")}`,
      );
    }
    ...
    record(
      "drc-after-jobs",
      true,
      "RunBoardJobDrc stops answering once the async export jobs have run — the markers step therefore runs before `jobs`",
      { knownGap: true },
    );
    record(
      "jobs",
      outcomes.every((o) => o.includes("done")),
      `${outcomes.length} exports (${asyncRuns} async)`,
    );
  }
  ...
  if (want("fpeditor")) {
    ...
    const pads = await kw(
      (key) =>
        [...window.__fpPcb.services.documents.footprint(key.slice("footprint:".length)).byType("KOT_PCB_PAD")].map((p) => ({
          id: p.id,
          n: p.proto.number,
    ...
    const boardAlive = await kw(async () => {
      try {
        await window.__fpPcb.services.documents.boardDoc.revision();
        return true;
      } catch (e) {
    ...
    record(
      "fpeditor",
      padX === "-0.9" && boardAlive === true,
      `pad edited in the library session, SaveDocument wrote ${padX} to the .kicad_mod, board untouched`,
    );
    await run("window.board");
    await page.waitForSelector('canvas[aria-label="board canvas"]', { timeout: 20000 });
  }
  ...
  if (want("schematic") || want("crossprobe") || want("erc")) {
    ...
    log(
      "schematic sheets:",
      JSON.stringify(await kw(() => window.__fpPcb.services.documents.sheets().map((s) => [s.name, s.file, s.page, s.children.length]))),
    );
  ...
  const waitSchRev = (after) =>
    page.waitForFunction((r) => window.__fpPcb.services.documents.schematicDoc.revision().then((v) => Number(v) > r), after, {
      timeout: 20000,
    });
  ...
  if (want("schematic")) {
    ...
    const bounds = await kw((key) => {
      const h = window.__fpPcb.host(key);
      h.setCamera({ x: 80e6, y: 140e6, zoom: 4e-6 });
      const c = document.querySelector('canvas[aria-label="schematic canvas"]');
      const a = h.screenToWorld(0, 0);
      const b = h.screenToWorld(c.clientWidth, c.clientHeight);
      return [a.x / 1e6, a.y / 1e6, b.x / 1e6, b.y / 1e6].map((v) => Math.round(v));
    }, rootKey);
    log("schematic camera framed, visible world (mm):", JSON.stringify(bounds));
    await page.waitForTimeout(300);
    const c0 = {
      line: await sc("KOT_SCH_LINE"),
      j: await sc("KOT_SCH_JUNCTION"),
      nc: await sc("KOT_SCH_NO_CONNECT"),
      l: await sc("KOT_SCH_LABEL"),
      g: await sc("KOT_SCH_GLOBAL_LABEL"),
      h: await sc("KOT_SCH_HIER_LABEL"),
      t: await sc("KOT_SCH_TEXT"),
      sym: await sc("KOT_SCH_SYMBOL"),
      sh: await sc("KOT_SCH_SHEET"),
    };
    ...
    await page.waitForFunction(() => /Symbol: click/.test(document.querySelector('[data-testid="tool-hint"]')?.textContent ?? ""), null, {
      timeout: 30000,
    });
    ...
    const sym = await kw((key) => {
      const s = [...window.__fpPcb.services.documents.sheet(key).byType("KOT_SCH_SYMBOL")].find(
        (x) => x.proto.referenceField?.text?.text === "R1",
      );
      return s
        ? {
            lib: `${s.proto.libId?.libraryNickname}:${s.proto.libId?.entryName}`,
            children: s.proto.definition?.items?.length,
            value: s.proto.valueField?.text?.text,
          }
        : null;
    }, sheetPath);
    const sheets = await kw(() =>
      window.__fpPcb.services.documents.sheets().map((s) => [s.name, s.file, s.children.map((c) => [c.name, c.file])]),
    );
    for (const l of await kw(() => (window.__fpPcb.stores.log?.getState().lines ?? []).slice(-40).map((e) => `[${e.level}] ${e.text}`)))
      log("app log:", l.slice(0, 300));
    log(
      "schematic counts",
      JSON.stringify(c0),
      "->",
    ...
    await page.screenshot({ path: `${shots}/schematic-tools.png` });
    const ok =
      c1.line === c0.line + 4 && // wire: 3 segments (two 90° bends), bus: 1
      c1.j === c0.j + 1 &&
      c1.nc === c0.nc + 1 &&
      c1.l === c0.l + 1 &&
      c1.g === c0.g + 1 &&
    ...
    record(
      "schematic",
      ok,
      `wire (3 seg) + bus + junction + no-connect + 3 labels + text + Device:R (${sym?.children} lib children) + sheet (hierarchy now ${JSON.stringify(sheets[0]?.[2])})`,
    );
    // save: KiCad's schematic SaveDocument is the known multi-handler dispatch bug
    const saveResult = await kw(async () => {
      try {
        await window.__fpPcb.services.documents.save("schematic");
        return "ok";
      } catch (e) {
        return e.message;
      }
    ...
    record(
      "schematic-save",
      saveResult === "ok",
      saveResult === "ok" ? `SaveDocument ok; files: ${files.join(", ")}` : `KiCad refused: ${saveResult}`,
    );
    record(
      "sheet-file",
      files.includes("proof_sheet.kicad_sch"),
      files.includes("proof_sheet.kicad_sch")
        ? "proof_sheet.kicad_sch created"
        : "SaveDocument did not create proof_sheet.kicad_sch for the sheet added through the API (the root file references it) — needs the C++ side to create files for new sheets on save",
      { knownGap: true },
    );
  }
  ...
  if (want("crossprobe")) {
    ...
    const symId = await kw(
      (key) =>
        [...window.__fpPcb.services.documents.sheet(key).byType("KOT_SCH_SYMBOL")].find((x) => x.proto.referenceField?.text?.text === "R1")
          ?.id ?? null,
      sheetPath,
    );
    if (symId) {
      ...
      const boardRef = await kw(
        (ids) => ids.map((id) => window.__fpPcb.services.documents.board().get(id)?.proto.referenceField?.text?.text?.text),
        boardSel,
      );
      ...
      record(
        "crossprobe",
        boardRef.includes("R1") && schSel.includes(symId) && active === "board",
        "footprint ↔ symbol by reference, both directions, jump switches editor",
      );
  ...
  if (want("annotate")) {
    ...
    const refs = await kw(
      (key) =>
        [...window.__fpPcb.services.documents.sheet(key).byType("KOT_SCH_SYMBOL")]
          .map((s) => s.proto.referenceField?.text?.text)
          .filter(Boolean)
          .sort(),
      sheetPath,
    );
  ...
  if (want("fields")) {
    ...
    await page.waitForTimeout(3000);
    const readBack = await kw(async () =>
      (await window.__fpPcb.services.schematic.fieldsTable()).rows.map((r) => [r.reference, r.fields.Value]),
    );
    await page.locator(".dialog .btn", { hasText: "Close" }).click();
    ...
    record(
      "fields",
      !!hit,
      `${rows} placements from GetSymbolFieldsTable; one edit committed through SetSymbolFields and read back on ${hit?.[0]}`,
    );
  }
  ...
  if (want("erc")) {
    ...
    const erc = await page
      .locator("[role=alert]")
      .innerText()
      .catch(async () => `${await page.locator(".marker-row").count()} markers`);
    log("ERC:", erc);
