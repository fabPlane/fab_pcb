    ...
    if (jsWanted) {
      ...
      if (js.run.state === "done" && s && s.routed > 0) {
        ...
        record(
          ...
          `JS router in the tab ${js.run.state} after ${(js.wallMs / 1000).toFixed(1)} s: ${js.run.error ?? (s ? `${s.routed}/${s.total} routed` : "no summary")}; board untouched (tracks ${js.before.tracks} -> ${js.after.tracks}), dialog ${reported ? "reported it" : "did NOT report it"}, tab stayed responsive (max ${js.maxLag} ms per evaluate)`,
          { knownGap: false },
        );
      }
    }
