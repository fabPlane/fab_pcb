...
const MENUS: { label: string; items: MenuEntry[] }[] = [
  ...
  },
  {
    label: 'Place',
    items: [
      'board.placeFootprint',
      'tools.libraryBrowser',
      'board.route',
      'board.placeVia',
      'board.drawZone',
      '-',
  ...
  { label: 'Route', items: ['board.route', 'board.placeVia', '-', 'board.autoroute'] },
  {
    label: 'Inspect',
    items: ['inspect.runDrc', 'inspect.runErc', 'board.severities', 'schematic.ercSeverities', '-', 'inspect.highlightNet', 'inspect.clearHighlight', 'inspect.nets', '-', 'inspect.crossProbe'],
  },
  {
    label: 'Tools',
    items: [
      ...
      'board.autoplace',
      'board.updateFootprints',
      'board.globalDeletion',
      '-',
      { label: 'Schematic' },
      'schematic.annotate',
      'schematic.fieldsTable',
      'schematic.assignFootprints',
      'schematic.updatePcb',
      'board.updateFromSchematic',
      '-',
      'tools.libraryBrowser',
      'board.openFootprintEditor',
      '-',
            ...
            <Menubar.Portal>
              <Menubar.Content className="menu-content" align="start" sideOffset={2}>
                {items.map((it, i) => {
                  ...
                  if (!('run' in it))
                    return (
                      <Menubar.Label key={`label-${i}`} className="menu-label">
                        {it.label}
                      </Menubar.Label>
                    );
                  const cmd = it;
