...
try {
  ...
  for (const f of b) if (!a.includes(f)) { console.error(`missing in src/gen: ${f}`); problems++; }
  for (const f of a) if (!b.includes(f)) { console.error(`stale in src/gen (not generated any more): ${f}`); problems++; }
  for (const f of a) {
    if (!b.includes(f)) continue;
    const [x, y] = await Promise.all([readFile(join(GEN_DIR, f), "utf8"), readFile(join(fresh, f), "utf8")]);
    if (x !== y) { console.error(`differs: src/gen/${f}`); problems++; }
  }
  const pinned = (await readFile(join(PKG_DIR, "KICAD_COMMIT"), "utf8").catch(() => "")).trim();
  const head = kicadCommit(src);
  if (pinned !== head) { console.error(`KICAD_COMMIT is ${pinned || "(missing)"} but ${src} is at ${head}`); problems++; }
} finally {
