...
if (fabRouterSource) {
  const destination = join(output, "private", "fab_router");
  await mkdir(join(destination, "spec"), { recursive: true });
  await cp(join(fabRouterSource, "package.json"), join(destination, "package.json"), { preserveTimestamps: true });
  await cp(join(fabRouterSource, "src"), join(destination, "src"), { recursive: true, preserveTimestamps: true });
  await cp(join(fabRouterSource, "spec", "types"), join(destination, "spec", "types"), {
    recursive: true,
...
console.log(
  `FabPlane PCB bundle: ${output} (${basename(cli)}, ${await countLibraries(footprints, ".pretty")} footprint libraries, ${await countLibraries(symbols, ".kicad_sym")} symbol libraries${fabRouterSource ? ", fab_router included" : ""}${jsAutorouterSource ? ", js_autorouter rollback included" : ""})`,
);
