...
For rollback, set `FP_PCB_CAPACITY_ROUTER=js-autorouter` and provide `JS_AUTOROUTER_MODULE`. The old
adapter remains present but is no longer the default. `laser-prefab` remains refused by both
adapters until fixed free-via claiming is implemented.
...
| Route                                                                                                     | Effect                                                                                                                                                                                                                                                                                                                                                        |
| --------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /sessions/:id/route` `{router:"js"\|"freerouting", options?, freerouting?, refillZones?, message?}` | `RefillZones` -> `SaveDocument` -> extract -> route -> apply (one commit, "Autoroute (<router>): n connections") -> `SaveDocument` on the session's open board; `202 {job}`; `400` when Freerouting is asked for but missing. A final-save failure leaves the route in KiCad memory but reports the job as failed so it is not mistaken for a durable result. |
